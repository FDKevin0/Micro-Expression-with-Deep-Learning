...
from models import VGG_16, VGG_16_test
...
# vgg_model = VGG_16('vgg16_weights_tf_dim_ordering_tf_kernels.h5')
# vgg_model = VGG_16()
...
if tensorboard_flag == 1:
	tensorboard_path = "/media/viprlab/01D31FFEF66D5170/tensorboard"

tot_mat = np.zeros((n_exp,n_exp))
spatial_weights_name = 'vgg_spatial_16a_smic.h5'
temporal_weights_name = 'temporal_16a_smic.h5'
for sub in range(subjects):
	...
	if train_spatial_flag == 1 and train_temporal_flag == 1:
		...
		# Spatial Training
		if tensorboard_flag == 1:
			vgg_model.fit(X, y, batch_size=32, epochs=10, shuffle=True, callbacks=[tbCallBack2])
		else:
			vgg_model.fit(X, y, batch_size=32, epochs=10, shuffle=True)
		...
		# Temporal Training
		if tensorboard_flag == 1:
			temporal_model.fit(features, Train_Y, batch_size=32, epochs=40, callbacks=[tbCallBack])
		else:
			temporal_model.fit(features, Train_Y, batch_size=32, epochs=40)	

		temporal_model.save_weights(temporal_weights_name)

		# Testing
		output = model.predict(test_X, batch_size = 32)
		features = output.reshape(int(output.shape[0]/10), 10, output.shape[1])
		predict = temporal_model.predict_classes(features, batch_size=32)
		...
		# Testing
		predict = vgg_model.predict_classes(test_X, batch_size = 32)
		...
		# Temporal Training
		if tensorboard_flag == 1:
			temporal_model.fit(Train_X, Train_Y, batch_size=32, epochs=40, callbacks=[tbCallBack])
		else:
			temporal_model.fit(Train_X, Train_Y, batch_size=32, epochs=40)	

		temporal_model.save_weights(temporal_weights_name)

		# Testing
		predict = temporal_model.predict_classes(Test_X, batch_size = 32)

	elif svm_flag == 1 and finetuning_flag == 0:
		# no finetuning

		X = vgg_model.predict(X, batch_size=32)
		y_for_svm = np.argmax(y, axis=1)

		svm_classifier.fit(X, y_for_svm)

		test_X = vgg_model.predict(test_X, batch_size=32)
		...
		# Testing
		predict = vgg_model.predict(test_X, batch_size = 32)		
		Test_Y_gt = np.repeat(Test_Y_gt, 10, axis=0)
	##############################################################
